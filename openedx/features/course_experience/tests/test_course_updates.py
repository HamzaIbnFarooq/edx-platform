...
from openedx.features.course_experience.tests import CourseUpdatesTestCase


class TestCourseUpdatesUtils(CourseUpdatesTestCase):
    ...
    def test_replace_urls(self):
        ...
        key = self.course.id
        updates = get_ordered_updates(self.request, self.course)
        self.assertEqual(updates[0]['content'],
                         f"<img src='/asset-v1:{key.org}+{key.course}+{key.run}+type@asset+block/img.png'>")
